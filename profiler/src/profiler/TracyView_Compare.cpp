...
extern double s_time;
    ...
    void View::FindZonesCompare()
    {
        ...
        if( !m_compare.match[1].empty() )
        {
            ...
            {
                if( m_compare.second->GetDefaultCtx().GetZonesForSourceLocation( *it ).zones.empty() )
                {
...
void View::DrawCompare()
{
    ...
    if( !m_worker.GetDefaultCtx().AreSourceLocationZonesReady() || !m_compare.second->GetDefaultCtx().AreSourceLocationZonesReady() )
    {
        const auto ty = ImGui::GetTextLineHeight();
        ImGui::PushFont( g_fonts.normal, FontBig );
    ...
    else
    {
        ...
        if( m_compare.compareMode == 0 )
        {
            ...
            if( ImGui::TreeNodeEx( "Matched source locations", ImGuiTreeNodeFlags_DefaultOpen ) )
            {
                ...
                {
                    auto& srcloc = m_compare.second->GetSourceLocation( v );
                    auto& zones = m_compare.second->GetDefaultCtx().GetZonesForSourceLocation( v ).zones;
                    ImGui::PushID( -1 - idx );
                    ImGui::PushStyleVar( ImGuiStyleVar_FramePadding, ImVec2( 0, 0 ) );
        ...
        if( ImGui::TreeNodeEx( "Histogram", ImGuiTreeNodeFlags_DefaultOpen ) )
        {
            ...
            if( m_compare.compareMode == 0 )
            {
                auto& zoneData0 = m_worker.GetDefaultCtx().GetZonesForSourceLocation( m_compare.match[0][m_compare.selMatch[0]] );
                auto& zoneData1 = m_compare.second->GetDefaultCtx().GetZonesForSourceLocation( m_compare.match[1][m_compare.selMatch[1]] );
