...
void View::DrawTimeline()
{
    ...
    if( m_gpuInfoWindow )
    {
        const auto px0 = ( m_worker.GetGpuExtra(*m_gpuInfoWindow).CpuStart() - m_vd.zvStart ) * pxns;
        const auto px1 = std::max( px0 + std::max( 1.0, pxns * 0.5 ), ( m_worker.GetGpuExtra(*m_gpuInfoWindow).CpuEnd() - m_vd.zvStart ) * pxns );
